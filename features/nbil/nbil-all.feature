Feature: NBIL Workflow
  ...
  @sanity@e2e
  Scenario Outline: <ScenarioNumber>New Opportunity---NBIL---IT-<Insurance Type> --- IST - <Insurance Subtype> --- Templates - <template1>, <template2>, <template3>
   ...
   # ---- NBIL Request 4 ----
    When I submit a new NBIL request
    And I select the valid "<carrier>" for "<template1>"
    And I validate the email page, template data, Upload the "set4" files and send the email for "<template1>" with "<message>" for "<Insurance Type>" and "<Insurance Subtype>"
    ...
    Then I validate the email body details with updated values for "<Insurance Type>", "<requestType>"
    And I close the "NBIL" tab
    And I validate the NBIL email is sent and "set4" files is uploaded
    And I close the "Files" tab
    When I resend NBIL request with carrier "<carrier>" and template "<template1>" and upload attachments and close the flow
    And I validate that uploaded files are not visible in Files related list
    And I close the "Files" tab
    ...
    When I submit a new NBIL request
    And I select the valid "<carrier>" for "<template2>"
    And I validate the email page, template data, Upload the "set5" files and send the email for "<template2>" with "<message>" for "<Insurance Type>" and "<Insurance Subtype>"
    ...
    Then I validate the email body details with updated values for "<Insurance Type>", "<requestType>"
    And I close the "NBIL" tab
    And I validate the NBIL email is sent and "set5" files is uploaded
    And I close the "Files" tab
    When I resend NBIL request with carrier "<carrier>" and template "<template2>" and upload attachments and close the flow
    And I validate that uploaded files are not visible in Files related list
    And I close the "Files" tab
    ...
    When I submit a new NBIL request
    And I select the valid "<carrier>" for "<template3>"
    And I validate the email page, template data, Upload the "set6" files and send the email for "<template3>" with "<message>" for "<Insurance Type>" and "<Insurance Subtype>"
    ...
    Then I validate the email body details with updated values for "<Insurance Type>", "<requestType>"
    And I close the "NBIL" tab
    And I validate the NBIL email is sent and "set6" files is uploaded
    And I close the "Files" tab
    When I resend NBIL request with carrier "<carrier>" and template "<template3>" and upload attachments and close the flow
    And I validate that uploaded files are not visible in Files related list
  ...
  |TC_004        | Tax Liability      | Tax Credit                 | ANV      | Test Email   | 100        | Patricia Urraca  | 900             | Progeny Health LLC                                  | The company provides Utilization and healthcare services in the U.S.                 | COB                | NSI                  | Altheimer & Gray      | 108        | NBIL Request- No NDA | Joint NDA and NBIL Request | NBIL Request- Pre-Cleared NDA | Test Purchase Agreement |35                | 300                | Atrium             | QofE                |Aird & Berlis                       |BRG            |Aprio      |ANV Underwriters                       |NBIL       | Test limit Comments   | Test other requested terms  |
  |TC_005        | Tax Liability      | M&A                        | CFC      | Test Email   | 100        | Patricia Urraca  | 800             | A&M Home Services                                   | It is for New Single-Family Housing Construction (except For-Sale Builders)          | 12 PM              | Both LSI & NSI       | Atrium                | 109        | NBIL Request- No NDA | Joint NDA and NBIL Request | NBIL Request- Pre-Cleared NDA | Test Purchase Agreement |40                | 400                | Abrams Fensterman  | Reviewed            |Akerman Senterfitt                  |Blythe         |Baker Tilly| CFC Underwriting Limited              |NBIL       | Test limit Comments   | Test other requested terms  |
